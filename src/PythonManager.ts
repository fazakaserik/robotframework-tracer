...
async function checkPythonPackages(context: vscode.ExtensionContext): Promise<void> {
    ...
    if (pythonExtension.exports && pythonExtension.exports.environment) {
        try {
            activePythonEnvironment = await pythonExtension.exports.environment.getEnvironmentDetails(
                (
        ...
        vscode.window.showErrorMessage('Python extension does not support the required API.');
    }
    ...
    for (const requirement of requirements) {
        try {
            ...
            cp.execSync(`${activePythonEnvironment.interpreterPath} -m pip show ${requirementName}`, { encoding: 'utf-8' });
        } catch {
    ...
    if (missingPackages.length > 0) {
        const installMsg = `The following python packages are missing: ${missingPackages.join(', ')}. Install now?`;
        const choice = await vscode.window.showInformationMessage(installMsg, 'Yes', 'No');
        if (choice === 'Yes') {
            console.log("installing packages")
            installPythonPackages(activePythonEnvironment.interpreterPath, missingPackages);
        } else {
            vscode.window.showErrorMessage("Required Python packages were not installed.");
        }
    } else {
        vscode.window.showInformationMessage("All required Python packages are installed.");
    }
}
    ...
    vscode.window.withProgress({
        ...
        return new Promise<void>((resolve, reject) => {
            cp.exec(cmd, (error, stdout, stderr) => {
                if (error) {
                    vscode.window.showErrorMessage(`Failed to install packages: ${stderr}`);
                    reject();
                } else {
